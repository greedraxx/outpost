...
import { updatePodcast, durationToSeconds } from "@/lib/supabase-queries"
...
export default function EditPodcastPage() {
  ...
  useEffect(() => {
    const checkAuthAndLoadPodcast = async () => {
      ...
      // Load podcast
      const { data: podcast, error: loadError } = await supabase
        .from('podcasts')
        .select('*')
        .eq('id', podcastId)
        .single()
  ...
  if (isCheckingAuth || isLoadingPodcast) {
    return (
        ...
        <div className="max-w-[1200px] mx-auto px-8">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-[42px] font-bold text-foreground">Edit Podcast</h1>
            </div>
            <Button
